...
enum states {INIT, off, on, on_release, transiton, timer, temp } state; //TODO: finish the enum for the SM
long input_min = 0;
long input_max = 1023;
long output_min = 0;
long output_max =16;
...
void Tick() {
  ...
  switch(state) {
    ...
    if(GetBit(PINC, 5) != 0){      //If the button is pressed, go to the off_release state
      state = transiton; 
  ...
  switch(state) {
    ...
    case off:
    PORTD = SetBit(PORTD,2,1); // RBG LED stays Green
    //PORTD = SetBit(PORTD,4,0); //reg led
    //PORTD = SetBit(PORTD,3,0);
  
    outNum(16); //display nothing bits are all zero
    break;

    case on:
    PORTD = SetBit(PORTD,2,1); // RGB LED stays Green
    break;
    ...
    case transiton:
    //outNum(map(ADC_read(17), input_min, input_max,  output_min, output_max)); //turn off display
    break;
