...
const unsigned long motor_task_period = 1;
    ...
    switch(state){
        ...
        case PWM_MOTOR_PT1: // MOTOR IS ON IN THIS STATE
         if(pot_percentage == 0 && button_pressed == true){
            ...
            if(PWM_motor_counter < pot_percentage){
                PORTB = SetBit(PORTB, 1, 1); //motor is on
                PORTB = SetBit(PORTB, 2, 0);  
                PWM_motor_counter++;
            }
            else{
