...
unsigned long input_min_PT2 = 50;
unsigned long input_max_PT2 = 800;
unsigned long output_min_PT2 = 1999;
unsigned long output_max_PT2 = 3999;
unsigned long joystick_movement_y_axis;
...
const unsigned long servo_task_period = 20;
...
enum servo_States {SERVO_INIT, SERVO_PT1 };
...
int main(void) {
    ...
    tasks[i].period = servo_task_period;
    tasks[i].elapsedTime = tasks[i].period;
    tasks[i].TickFct = &Tick_servo;
    ++i;
...
int Tick_joystick(int state){
  switch(state){
    ...
    case JOYSTICK_PT1:
    joystick_movement_y_axis = map(ADC_read(1), input_min_PT2, input_max_PT2, output_min_PT2, output_max_PT2);
...
int Tick_stepper_motor(int state){
  switch(state){
    ...
    //serial_println(joystick_movement_x_axis);
...
int Tick_servo(int state){
  serial_println(joystick_movement_y_axis);
  switch(state){
    case SERVO_INIT:
    state = SERVO_PT1;
    break;
    ...
    case SERVO_PT1:
    //serial_println("hi");
    // Setup PWM for servo control on Pin 9 (OC1A)
TCCR1A |= (1 << WGM11) | (1 << COM1A1); // Use channel A
TCCR1B |= (1 << WGM12) | (1 << WGM13) | (1 << CS11); // Fast PWM, prescaler 8

ICR1 = 39999; // 20 ms PWM period

// Set OCR1A within 2000 to 4000 for typical servo positions
OCR1A = joystick_movement_y_axis; // Adjust for desired servo position

// Ensure PB1 is configured as an output
DDRB |= (1 << PB1);
